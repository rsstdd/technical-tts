...
| 4. Voices resolved before planning | Met. `voice_gate::resolve_speakers` runs between validation and planning, keys its work by profile identity so one profile is read once however many speakers name it, loads each through the existing fail-closed gate, and `BuildRequest::voice_profile_root` is required rather than optional. `RenderPlan::for_lesson` refuses with `PlanError::UnresolvedSpeaker` rather than deriving a key for a speaker the caller did not resolve, so the guarantee is the planner's own and not a convention callers must keep. The profile-keyed half is now observed rather than argued: `resolve_by_profile` takes its loader as a parameter, and `t1_e1_one_voice_profile_is_loaded_once_however_many_speakers_name_it` counts the calls over the cache-identity fixture, whose two speakers share one profile across six segments. Confirmed live by keying deduplication on the speaker instead of the profile: the test failed naming the profile twice, and every other test in the workspace — including the ten in `voice_rights` and the thirty-five in `walking_skeleton` — still passed, which is what the deviation below had recorded as the gap. |
...
- **"Resolved once" is now observed, beside the module rather than through the pipeline.** This
  was recorded here as a limitation: `t4_e1_two_speakers_may_share_one_voice_profile` asserts only
  that a shared profile resolves, because no seam reachable from `study-tts-testkit` can count
  reads, and a rewrite resolving per speaker would have passed the whole suite. The countable seam
  is `voice_gate::resolve_by_profile`, which takes the loader as a parameter — the returned map
  cannot reveal load multiplicity, so nothing weaker than counting the calls would have closed it —
  and `t1_e1_one_voice_profile_is_loaded_once_however_many_speakers_name_it` counts them from a
  compiled-in lesson, touching no filesystem. The T4 keeps its narrower claim and its name; its doc
  comment now points at the counted half rather than saying nothing observes it. No public surface
  moved and no behavior changed: `resolve_speakers` keeps its signature, its callers, and its
  refusals.
