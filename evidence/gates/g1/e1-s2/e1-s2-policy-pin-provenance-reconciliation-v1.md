...
- Candidate revision: working tree on `main` with the issue #58 policy-pin follow-up
...
4. Traceability describes role/style as a two-sided but non-mechanized mirror and preserves the
   issue #58 trigger: revisit when ADR-0001 gains an exact normative list or real drift occurs.
...
| 4. Remaining limitation | Met. The traceability row names both T3 controls and states why no role/style expected array exists and what triggers reconsideration. Issue #58 remains open on that scope. |
| 5. Verification and provenance | Pending approval only. Every implementation check below passes. While this record is `Proposed`, the provenance checker correctly reports exactly the two v15 mismatches this record names; its accounting rows cannot take effect before approval. |
...
so it would give a false impression of mechanized agreement. Issue #58 remains open until
ADR-0001 supplies that list or a real drift demonstrates a narrower independent control.
...
| `python3 scripts/check-evidence-provenance.py` | Expected pre-approval red: exactly the two v15 pins named above; zero remains required after approval |
| `git diff --check` | Pass |
...
|---|---|---|---|
| Engineering owner | Ross Todd | Pending — approve the two test-only policy pins, their isolated mutation evidence, and accounting for v15's `lesson.rs` pin without changing production behavior or the baseline conclusion | |
| Project owner | Ross Todd | Pending — approve the updated traceability route, keep issue #58 open for the role/style limitation, and account for v15's traceability pin without superseding v15 | |
