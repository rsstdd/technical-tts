...
is the one record issue #80 keeps: this version answers **AC5** (render through the real worker and
keep the package) and **AC7** (measured render time and cache growth). **AC6**, the M2 acceptance
listening session, is opened in §Listening below and is not taken. Its findings are added here
when it is, and this record is accepted once, at M2, against the bytes that gate approves.
...
## Listening — AC6, opened and not taken

Issue #80's remaining criterion is the human review against
...
It has not been taken, and one thing about this material has to reach the listener before it is:
**the master is quiet on purpose.** It sits at `-27.02` LUFS, roughly 11 dB below an ordinary
spoken-word delivery level, because that is the loudest this voice reference permits beneath a
`-1.0 dBTP` ceiling. A listener will reach for the volume control. That is the recorded constraint
`ADR-0001-D012` names, not a defect in the audio, and a finding that says only "too quiet" records
something already known. What the session is for is everything audible *after* the volume is
raised.

Findings are recorded in this section when the session is taken.
