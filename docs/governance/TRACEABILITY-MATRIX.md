...
| Canonical reviewed lesson only | E1-S2, E3-S3 | `t1_e1_each_lesson_invariant_has_a_distinct_error`, `t1_e1_unreviewed_lesson_fails_before_worker_start`, `t1_e1_display_text_never_enters_synthesis_request`, `t2_e1_unicode_and_protected_terms_survive_round_trip`, and `t2_e1_plan_is_stable_for_identical_lesson_input`; declared-voice resolution and one voice per declared speaker in `crates/study-tts-runtime/src/voice_gate.rs`; the closed `SegmentRole` and `DeliveryStyle` vocabularies and the `MIN_RECALL_RESPONSE_MS` and `MAX_RECALL_RESPONSE_MS` recall interval in `crates/study-tts-core/src/lesson.rs`, transcribed from ADR-0001 §3.2, §5.1, §8.1, and §13.2, which the two must agree with; display/spoken audit carried into the plan; located diagnostics | G1/G3 |
