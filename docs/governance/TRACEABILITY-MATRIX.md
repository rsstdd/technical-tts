...
| Canonical reviewed lesson only | E1-S2, E3-S3 | `t1_e1_each_lesson_invariant_has_a_distinct_error`, `t1_e1_unreviewed_lesson_fails_before_worker_start`, `t1_e1_display_text_never_enters_synthesis_request`, `t2_e1_unicode_and_protected_terms_survive_round_trip`, and `t2_e1_plan_is_stable_for_identical_lesson_input`; declared-voice resolution and one voice per declared speaker in `crates/study-tts-runtime/src/voice_gate.rs`; `t3_e1_recall_response_interval_matches_adr` pins the exact ADR-0001 §13.2 endpoints in `crates/study-tts-core/src/lesson.rs`, and `t3_e1_provisional_lesson_resource_ceilings_match_walking_skeleton_document` pins its ten exact lesson ceilings to `docs/architecture/WALKING-SKELETON.md` §Provisional resource ceilings; the closed `SegmentRole` and `DeliveryStyle` vocabularies form a two-sided mirror between `lesson.rs` and ADR-0001 §3.2, §5.1, and §8.1, but remain non-mechanized because the ADR supplies no exact normative list — revisit when it gains one or real drift occurs; display/spoken audit carried into the plan; located diagnostics | G1/G3 |
