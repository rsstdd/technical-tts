...
11. Derive `transcript.txt`, `transcript.vtt`, and `chapters.ffmetadata` from the frame boundaries the assembly write loop recorded, escaping reviewed `display_text` so it cannot terminate a cue or open an FFMETADATA key. `crates/study-tts-runtime/src/timeline.rs` owns all three and names this step in return: `timeline::chapters` writes exact frame counts under `TIMEBASE=1/24000`, while `timeline::captions` and `timeline::timestamp` write a **floored millisecond projection** of each boundary, because WebVTT has no sub-millisecond representation and a 24 kHz frame is 1/24 of one. That projection departs from ADR-0001 §17.12 under the approved [`ADR-0001-D010`](../adr/deviations/ADR-0001-D010-webvtt-millisecond-caption-projection.md), which carries no expiry because the constraint is the format §13.5 names; the exact boundaries stay in `manifest.json`, which is the compensating control that record rests on rather than compliance.
