...
| `run-report` / `1.0-skeleton` | T-CLI | `study-tts report`, E6 measurement | `schemas/run-report-v1.schema.json`; `study_tts_runtime::RunReport` | as above | `t3_e1_published_schema_required_fields_match_the_recorded_surface`; `t3_e2_every_published_run_report_number_names_its_unit`; `t1_e2_run_report_units_and_missing_values_follow_schema` | None. Nothing in it reaches a synthesis key, a plan hash, or a package identity | `-skeleton` retained: later E2-S4 steps add stage durations and per-segment rows, so the label must not claim a stability they will take away. Added by `E2-S4-INTERFACE-CHANGE-001`; nothing consumed it before, so nothing migrates |
