...
|---|---|---|
| 2026-09-10 | **`run-report` moves to `3.0`, layout `3.0-skeleton`.** Accepted ADR-0002's waiver retains "per-run wall time, RTF, peak-RAM, thread-budget, worker identity, and hardware identity" in every run report until it expires, and the report carried only the first three. Three required fields answer the rest: `worker_bundle_hash`, `hardware_environment_id`, and `resources.thread_budget`. `ADR-0001-D002` summarizes the sentence as one "environment identity", but it is approved *through* ADR-0002 and cannot merge what the controlling decision separates, so the two identities are published separately. **The major moves rather than folding into `2.0`.** The earlier amendment folded the `1.0` draft in because "no accepted build has published the earlier draft"; `2.0` is published on `main`, and `E2-INTERFACE-CHANGE-001` settled that an unsigned version is kept out of the charter's effective history without becoming reusable for a different layout — manifest `3.0-skeleton` was "implemented, merged, and superseded without being authorized" and its next breaking change still moved to `4.0-skeleton`. `schemas/run-report-v2.schema.json` is retired and `run-report-v3.schema.json` replaces it. A `2.0-skeleton` report stays **readable and never reusable**: `manifest::validate_run_report` decodes it through a frozen `LegacyRunReport` and applies the same manifest joins, returning `ReportLayoutRead::Superseded` so `preview::current_for_build` rebuilds instead of erroring — returning an error there would make every package written before this change unrecoverable rather than stale. That frozen decoder is the fourth in this tree, which answers **G-B** of `E2-INTERFACE-CHANGE-001` in the "read it" direction for this contract. `t4_e2_a_package_with_a_superseded_run_report_is_preserved_and_rebuilt` proves both halves. Package identity moves, because the manifest checksums the report; no synthesis, verification, plan, takes, or cache identity does, and no cache entry is stranded. | |
