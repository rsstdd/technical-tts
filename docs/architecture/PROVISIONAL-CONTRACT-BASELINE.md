...
| `job_state` / `1.0` | T-CORE/T-RUNTIME; `JobDocument` and `JobState` plus `JobRepository` and `JobOwnership` | Preview orchestration and resume | `InMemoryJobRepository`, `InterruptingJobRepository`; `run_job_repository_contract_scenario`; strict document written as `job.json` beside the retained `lesson.json`, `plan.json`, and `events.ndjson`; `validate_preview_selection` compares a recorded preview completion and plan with the reconciled selected output | Recorded cache keys, audio digests, and the selected package are durable state; this document does not define synthesis or verification keys | Replaced the provisional `e0.job-state.0.1` snapshot at E2-S1 (`E2-S1-INTERFACE-CHANGE-001`); E4-S4 and E5 recovery extend it |
